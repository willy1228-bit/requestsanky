...
OUTPUT_FILE = f"{ROOT_ADDRESS[:6]}_trace.csv" if ROOT_ADDRESS else "trace.csv"
...
def filter_and_parse_transactions(txs, current_address, depth, from_address):
    ...
    for tx in txs:
        ...
        parsed.append({
            ...
            "To": tx["to"],
            "方向": direction,
            "金額 (ETH)": round(int(tx["value"]) / 1e18, 8),
...
def start_tracking(root_address):
    ...
    progress_bar.empty()
    st.success(f"✅ 追蹤結束，共 {len(df_trace)} 筆交易")
...
if os.path.exists(OUTPUT_FILE):
    ...
    if not df.empty:
        ...
        def make_trace(data, top_n):
            data = data.sort_values(by="金額 (ETH)", ascending=False).head(top_n)
            return dict(
        ...
        fig.update_layout(
            title_text=f"ETH 流向圖（轉入 L{st.session_state.in_layer}，轉出 L{st.session_state.out_layer}，Top {top_n}）",
            font_size=10
        )
        st.plotly_chart(fig, use_container_width=True)

# ======== 新增：從現有資料選擇錢包再追蹤（顯示層級與交易數） ========
if not st.session_state.df_trace.empty:
    ...
    if st.button("以選擇的錢包為起點繼續追蹤"):
        ...
        OUTPUT_FILE = f"{ROOT_ADDRESS[:6]}_trace.csv"
        st.session_state.df_trace = pd.DataFrame()
        if os.path.exists(OUTPUT_FILE):
